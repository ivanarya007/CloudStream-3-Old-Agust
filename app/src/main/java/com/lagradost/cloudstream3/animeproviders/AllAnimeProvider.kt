...
class AllAnimeProvider : MainAPI() {
    ...
    override suspend fun getMainPage(): HomePageResponse {
        ...
        val testa = app.get(random).text
        val jsonran = parseJson<ExampleJson2KtKotlin>(testa)
        val aa = jsonran.data?.queryRandomRecommendation?.map {
            AnimeSearchResponse(
                it.name!!,
                "$mainUrl/anime/${it.Id}",
                this.name,
        ...
        items.add(HomePageList("Random", aa!!))
